...
本文档面向后续接手该仓库的人或大模型，目标是基于当前源码说明项目已经实现了什么、主流程如何运转、各模块分别负责什么，以及哪些能力仍处于原型或基础版阶段。
...
llm_brain 是一个基于 LangGraph 的通用 Agent 原型，围绕“特征提取 -> 任务规划 -> 子任务执行 -> 工具调用 -> 反思推进”的主循环，已经接入记忆、技能、请求级日志追踪、运行时快照，以及基础版真实 MCP stdio transport。
...
5. agent 节点执行当前子任务，统一选择 Prompt 技能与候选工具，必要时对失败工具做再路由。
6. 如果模型返回 tool_calls，则进入 ToolNode 执行工具，之后再回到 agent。
7. reflect_and_advance 节点根据 expected_outcome 与实际输出决定 continue、retry 或 ask_user。
8. 如果 retry 路径持续失败，系统可在第一次 retry 时调用 planner 对失败子任务做一次更小粒度的重规划。
9. 成功完成后返回最后结果；若重试超限或明确阻断，则返回提示用户介入的消息。
...
- 超时与取消目前仍属于逻辑取消，不是底层线程或进程的硬终止
- 技能与工具路由仍以关键词和阈值匹配为主，没有向量召回或更强语义检索
...
| llm_manager.py | 模型实例管理、结构化日志、控制台阶段输出 |
...
| /load_mcp <config_name.json\|config_name.yaml\|absolute_path\|server.py\|stdio:command ...> | 加载配置式 MCP 工具或真实 stdio MCP server 并注册工具 |
...
| /request_summary <request_id> | 展示该请求的状态、指标、checkpoint、快照和关联记忆 |
| /recent_requests [limit] | 展示最近若干次请求的聚合摘要 |
...
| get_recent_request_summaries(limit=10) | 聚合最近请求的摘要视图 |
| convert_memory_to_skill(memory_id) | 将历史记忆转换为 Markdown 技能 |
| load_skill(skill_name) | 手动加载 Python 或 Markdown 技能 |
| load_mcp_server(server_ref) | 加载配置式 MCP 工具或真实 stdio MCP server 并注册工具 |
...
- 写步骤级记忆
- 更新 retry_counts 和 replan_counts
- 写 reflection_completed、subtask_advanced、subtask_replanned、agent_blocked、agent_completed 等 checkpoint
- 在推进、完成或阻断时写运行时状态快照
...
AgentCore 当前已经提供两类聚合视图：

- get_request_summary(request_id)：整合 snapshots、checkpoints、memory 和记指标
- get_recent_request_summaries(limit)：扫描 runtime_state/snapshots 下的 request_id 目录并按更新时间排序

当前汇总指标包括：

- total_duration_ms
- llm_call_count / llm_error_count / llm_total_duration_ms
- tool_call_count / tool_success_count / tool_failure_count / tool_rejection_count / tool_cancelled_count
- tool_hit_rate
- retry_count
- subtask_count
...
- 对外提供 MCP server 的加载、列举、刷新、卸载接口
...
| list_memories_by_request_id(request_id) | 按 request_id 列出关联记忆 |
...
- load_full_memory() 会返回 conv_id、request_id、memory_type 和 quality_tags
...
当前仍主要支持 stdio transport，且连接管理仍是基础版：已支持连接复用、自动重连与显式刷新/关闭，但尚未补工具刷新订阅和更细粒度健康检查。

### 8.8 system_mcp_server

mcp_servers/system_mcp_server.py 是仓库内置的独立 Python MCP server，当前提供的工具包括：

- execute_terminal_command：执行终端命令
- get_system_info：查看主机、平台、Python 与磁盘等系统信息
- get_mcp_security_policy：查看当前命令/路径白名单和审计日志位置
- inspect_file_system_path：查看文件或目录信息与文本预览

当前该 server 还带有一层基础安全控制：
...
- test_cli_and_integration.py：CLI 命令、主链成功/阻断集成、MCP CLI 命令
...
- 真实 MCP 已完成基础版 stdio transport 接入，但尚未补工具刷新订阅、资源同步和更细粒度的连接健康检查
- 超时和取消仍是逻辑层面，不是底层线程或进程的硬中止
...
- MCP 当前虽已接入真实 stdio transport，但仍缺少更完整的长连接工程化管理与资源同步机制
...
llm_brain 当前仍是原型工程，但已经不再只是最小骨架。它现在具备了可追踪的执行链、基础记忆闭环、统一技能路由、请求级可观测性、快照恢复，以及基础版真实 MCP stdio 接入能力。对后续接手者来说，最重要的不是把目标文档当作现状，而是先理解当前已经可靠落地的这些链路，再决定下一步要继续补的是 transport 工程化、检索能力，还是更强的执行稳定性。